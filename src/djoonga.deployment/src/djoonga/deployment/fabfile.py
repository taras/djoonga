...
def sync(source=None, target=None):
    ...
    def get_hash():
        random.seed()
        return 'temp_%s'%str(random.getrandbits(128))[0:8]    
    
    require('TABLE_SYNC_PATH', used_for='TABLE_SYNC_PATH is not set.')
